...
describe("collider transform buffer sync", () => {
    test("attached collider tracks its parent body after a step", () => {
        ...
        expect(collider.rotation()).toBeCloseTo(body.rotation(), 5);
    ...
    test("collider on a sleeping body keeps its resting transform", () => {
        ...
        world.createCollider(RAPIER.ColliderDesc.cuboid(10, 0.1), floor);
    ...
    test("parentless collider keeps its position across steps", () => {
        ...
        const collider = world.createCollider(
            RAPIER.ColliderDesc.cuboid(0.5, 0.5).setTranslation(4, 2),
        );
    ...
    test("collider.setTranslation is reflected immediately (buffer invalidated)", () => {
        const world = new RAPIER.World({x: 0, y: -9.81});
        const collider = world.createCollider(RAPIER.ColliderDesc.ball(0.5).setTranslation(0, 0));
        world.step();

        collider.setTranslation({x: 7, y: 8});
        const t = collider.translation();
    ...
    test("buffer reads match the WASM path right after creating colliders", () => {
        ...
        const colliders: any[] = [];
        for (let i = 0; i < 50; i++) {
            const body = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(i, 5));
