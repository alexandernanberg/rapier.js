...
function sceneWith(cuboids: number) {
    ...
    for (let i = 0; i < cuboids; i++) {
        const body = world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(i * 3, 0));
...
describe("debug render buffers", () => {
    test("produces lines with one RGBA color per vertex", () => {
        ...
        // Two floats per vertex, four color components per vertex.
        expect(buffers.vertices.length % 2).toBe(0);
        expect(buffers.colors.length).toBe((buffers.vertices.length / 2) * 4);
    ...
    test("a target grows for a bigger scene and reports the smaller length again", () => {
        ...
        const extra = world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(5, 0));
        world.createCollider(RAPIER.ColliderDesc.cuboid(0.5, 0.5), extra);
        world.step();
        ...
        expect(target.colors.length).toBe((small / 2) * 4);
