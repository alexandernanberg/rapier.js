...
describe("impulse joints", () => {
    test("a fixed joint holds a dynamic body against gravity", () => {
        ...
        const hung = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(1, 5));
        world.createCollider(RAPIER.ColliderDesc.ball(0.2), anchor);
        world.createCollider(RAPIER.ColliderDesc.ball(0.2), hung);

        world.createImpulseJoint(
            RAPIER.JointData.fixed({x: 1, y: 0}, 0, {x: 0, y: 0}, 0),
            anchor,
            hung,
            true,
        );
    ...
    test("a revolute joint keeps the bob at the anchor distance", () => {
        const world = new RAPIER.World(GRAVITY);
        const anchor = world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(0, 5));
        const bob = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(2, 5));
        world.createCollider(RAPIER.ColliderDesc.ball(0.2), bob);

        world.createImpulseJoint(
            RAPIER.JointData.revolute({x: 0, y: 0}, {x: -2, y: 0}),
            anchor,
            bob,
            true,
        );
