 ...
 * cross the boundary shows up here as unbounded growth. Slack the allocator is
 * already holding can absorb a small leak, so each workload is sized until an
 * injected leak clears it by a wide margin — a leak grows without bound, slack
 * does not. Correct workloads measure at exactly zero.
 */
...
function populate(world: RAPIER.World, count: number) {
    ...
    world.createCollider(RAPIER.ColliderDesc.cuboid(50, 0.1), ground);
    for (let i = 0; i < count; i++) {
        const body = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation((i % 10) * 2 - 10, 1 + i * 0.1),
        );
...
// NOTE: the 3D suite additionally covers the mass-property getters, which hand
// back `RawVector`/`RawSdpMatrix3` there. In 2D the same getters return plain
// numbers, so there is nothing for them to leak and no test worth writing.
describe("memory", () => {
    test("spawning and despawning bodies reaches a steady heap", () => {
        ...
        expectSteadyMemory(
            () => {
                ...
                for (let i = 0; i < 50; i++) {
                    const body = world.createRigidBody(
                        RAPIER.RigidBodyDesc.dynamic().setTranslation(i * 0.5 - 12, 3),
                    );
                    world.createCollider(RAPIER.ColliderDesc.ball(0.4), body);
                    spawned.push(body);
                }
    ...
    test("scene queries do not leak the raw objects they hand across", () => {
        ...
        const ray = new RAPIER.Ray({x: 0, y: 30}, {x: 0, y: -1});
    ...
    test("reading shapes back from colliders does not leak raw shapes", () => {
        ...
        const handles = [
            ...
            world.createCollider(RAPIER.ColliderDesc.cuboid(1, 2), body).handle,
            world.createCollider(RAPIER.ColliderDesc.roundCuboid(1, 2, 0.1), body).handle,
            world.createCollider(RAPIER.ColliderDesc.capsule(1, 0.5), body).handle,
            world.createCollider(
                RAPIER.ColliderDesc.convexHull(new Float32Array([0, 0, 1, 0, 1, 1, 0, 1]))!,
                body,
            ).handle,
            world.createCollider(
                RAPIER.ColliderDesc.compound(
                    [new RAPIER.Ball(0.5), new RAPIER.Cuboid(1, 1)],
                    [
                        {x: 0, y: 0},
                        {x: 2, y: 0},
                    ],
