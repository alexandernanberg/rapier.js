...
describe("Stale handles", () => {
    test("a stale body handle does not resolve to the body that recycled its index", () => {
        ...
        const a = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(1, 0));
    ...
    test("accessors on a removed body throw a JS error and leave the world usable", () => {
        const world = new RAPIER.World({x: 0, y: -9.81});
        const body = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(1, 0));
        ...
        expect(() => body.setLinvel({x: 1, y: 0}, true)).toThrow(/removed/);
        ...
        const other = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(2, 0));
    ...
    test("accessors on a removed joint throw a JS error", () => {
        ...
        const a = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(1, 0));
        const b = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(2, 0));
        const params = RAPIER.JointData.fixed({x: 0, y: 0}, 0, {x: 0, y: 0}, 0);
