import RAPIER, {init, scratch} from "@alexandernanberg/rapier2d/compat";
...
function createStack(world: RAPIER.World, count: number) {
    ...
    world.createCollider(RAPIER.ColliderDesc.cuboid(50, 0.1), floor);

    const bodies: RAPIER.RigidBody[] = [];
    for (let i = 0; i < count; i++) {
        const body = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(i * 1.5 - count, 0.6),
        );
...
describe("incremental transform buffer sync", () => {
    ...
    test("sleeping bodies stay in sync while other bodies keep moving", () => {
        ...
        bodies[0].setTranslation({x: bodies[0].translation(_v()).x, y: 8}, true);
        for (let i = 0; i < 200; i++) {
            world.step();
            expectBufferMatchesWasm(world);
        }
    ...
    test("moving a sleeping body without waking it refreshes body and collider", () => {
        ...
        body.setTranslation({x: 20, y: 3}, false);
        world.step();
    ...
    test("a collider repositioned on a sleeping body is refreshed", () => {
        ...
        collider.setTranslationWrtParent({x: 0, y: 4});
        world.step();
    ...
    test("a body created next to sleeping ones is written into the buffer", () => {
        ...
        const fresh = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(30, 5));
    ...
    test("recycled arena indices do not resurrect a removed body's transform", () => {
        ...
        const fresh = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(-40, 2));
        world.createCollider(RAPIER.ColliderDesc.ball(0.5), fresh);
        world.step();
    ...
    test("a fixed body recycling an active body's slot is written", () => {
        ...
        for (const b of bodies) b.setTranslation({x: b.translation(_v()).x, y: 20}, true);
        ...
        const fixed = world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(17, 4));
        ...
        expect(fixed.translation(_v())).toEqual({x: 17, y: 4});
        expectBufferMatchesWasm(world);
