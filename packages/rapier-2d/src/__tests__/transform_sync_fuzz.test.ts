...
describe("transform buffer fuzz", () => {
    for (const seed of [1, 2, 3]) {
        test(`buffered transforms match WASM under random churn (seed ${seed})`, () => {
            ...
            const vec = (s = 1) => ({x: rf(-s, s), y: rf(-s, s)});

            const world = new RAPIER.World(GRAVITY);
            const ground = world.createRigidBody(RAPIER.RigidBodyDesc.fixed());
            world.createCollider(RAPIER.ColliderDesc.cuboid(60, 0.1), ground);
            ...
            function audit(step: number) {
                ...
                world.bodies.forEach((b) => {
                    const wt = wasmRead(() => world.bodies.raw.rbTranslation(b.handle), 2);
                    const t = b.translation();
                    if (!near(t.x, wt[0]) || !near(t.y, wt[1]))
                        mismatches.push(
                            `step ${step}: body translation ${JSON.stringify(t)} vs wasm [${wt}]`,
                        );

                    const wv = wasmRead(() => world.bodies.raw.rbLinvel(b.handle), 2);
                    const v = b.linvel();
                    if (!near(v.x, wv[0]) || !near(v.y, wv[1]))
                        mismatches.push(`step ${step}: body linvel ${JSON.stringify(v)}`);

                    const wr = wasmRead(() => world.bodies.raw.rbRotation(b.handle), 1);
                    const r = b.rotation();
                    if (!near(r, wr[0]))
                        mismatches.push(`step ${step}: body rotation ${r} vs wasm ${wr[0]}`);
                });
                world.colliders.forEach((c) => {
                    const wt = wasmRead(() => world.colliders.raw.coTranslation(c.handle), 2);
            ...
            function spawn(fixed: boolean) {
                ...
                const p = vec(20);
                desc.setTranslation(p.x, Math.abs(p.y) * 0.3 + 0.6);
                const b = world.createRigidBody(desc);
                world.createCollider(RAPIER.ColliderDesc.cuboid(0.4, 0.4), b);
                bodies.push(b);
                return b;
            }
            ...
            for (let step = 0; step < 300; step++) {
                ...
                for (let m = 0; m < burst; m++) {
                    ...
                    switch (ri(9)) {
                        case 0:
                            b.setTranslation({x: rf(-25, 25), y: rf(0.5, 12)}, wake);
                            break;
                        case 1:
