        ...
        test(`buffered transforms match WASM under random churn (seed ${seed})`, () => {
            ...
            function audit(step: number) {
                ...
                world.bodies.forEach((b) => {
                    ...
                    if (!near(t.x, scratch[0]) || !near(t.y, scratch[1]))
                        mismatches.push(
                            `step ${step}: body translation ${JSON.stringify(t)} vs wasm [${scratch[0]}, ${scratch[1]}]`,
                        );
                    world.bodies.raw.rbLinvel(b.handle, scratch);
                    const v = b.linvel();
                    if (!near(v.x, scratch[0]) || !near(v.y, scratch[1]))
                        mismatches.push(`step ${step}: body linvel ${JSON.stringify(v)}`);
                    world.bodies.raw.rbRotation(b.handle, scratch);
                    const r = b.rotation();
                    if (!near(r, scratch[0]))
                        mismatches.push(`step ${step}: body rotation ${r} vs wasm ${scratch[0]}`);
                });
