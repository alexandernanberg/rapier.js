...
describe("audit regressions", () => {
    test("forEachActiveRigidBody visits every body even when the callback grows WASM memory", () => {
        ...
        for (let i = 0; i < 40; i++) {
            const body = world.createRigidBody(
                RAPIER.RigidBodyDesc.dynamic().setTranslation(i * 3, 10, 0),
            );
    ...
    test("the cached shape follows the shape mutators", () => {
        ...
        const box = world.createCollider(RAPIER.ColliderDesc.cuboid(1, 1, 1), body);
        box.setHalfExtents({x: 2, y: 3, z: 4});
        expect((box.shape as RAPIER.Cuboid).halfExtents).toEqual({x: 2, y: 3, z: 4});
        ...
        const round = world.createCollider(RAPIER.ColliderDesc.roundCuboid(1, 1, 1, 0.1), body);
    ...
    test("a removed body does not read another body's transform out of the buffer", () => {
        const world = new RAPIER.World(GRAVITY);
        const removed = world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(1, 2, 3));
        const collider = world.createCollider(RAPIER.ColliderDesc.ball(0.5), removed);
        world.step();
        expect(removed.translation()).toEqual({x: 1, y: 2, z: 3});

        world.removeRigidBody(removed);
        // Recycles the arena index the removed body held.
        const successor = world.createRigidBody(
            RAPIER.RigidBodyDesc.fixed().setTranslation(7, 8, 9),
        );
        ...
        expect(successor.translation()).toEqual({x: 7, y: 8, z: 9});
    ...
    test("collider queries against a removed collider miss instead of trapping", () => {
        ...
        const b = world.createCollider(RAPIER.ColliderDesc.ball(0.5).setTranslation(3, 0, 0));
        world.removeCollider(b, false);

        expect(a.castCollider({x: 1, y: 0, z: 0}, b, {x: 0, y: 0, z: 0}, 0, 10, true)).toBeNull();
        expect(a.contactCollider(b, 1)).toBeNull();
        world.free();
    });

    test("the vehicle controller survives its chassis being removed", () => {
        const world = new RAPIER.World(GRAVITY);
        ...
        expect(() => vehicle.updateVehicle(1 / 60)).not.toThrow();
        world.free();
    });
    ...
    test("a body pushed by the character controller reports its new velocity right away", () => {
        ...
        const box = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(1.2, 1.0, 0),
        );
        world.createCollider(RAPIER.ColliderDesc.cuboid(0.5, 0.5, 0.5).setDensity(1), box);
        ...
        const characterCollider = world.createCollider(
            RAPIER.ColliderDesc.cuboid(0.3, 0.5, 0.3),
            character,
        );
        ...
        controller.computeColliderMovement(characterCollider, {x: 1, y: 0, z: 0});
    ...
    test("a drifted quaternion is normalized rather than dropped", () => {
        const world = new RAPIER.World(GRAVITY);
        const s = Math.SQRT1_2 * 1.02; // 2% off unit length: outside the tolerance
        const fromDesc = world.createRigidBody(
            RAPIER.RigidBodyDesc.fixed().setRotation({x: 0, y: s, z: 0, w: s}),
        );
        const fromSetter = world.createRigidBody(RAPIER.RigidBodyDesc.fixed());
        fromSetter.setRotation({x: 0, y: s, z: 0, w: s}, false);
        const collider = world.createCollider(RAPIER.ColliderDesc.ball(1));
        collider.setRotation({x: 0, y: s, z: 0, w: s});

        for (const rot of [fromDesc.rotation(), fromSetter.rotation(), collider.rotation()]) {
            expect(rot.y).toBeCloseTo(Math.SQRT1_2, 5);
            expect(rot.w).toBeCloseTo(Math.SQRT1_2, 5);
        }

        // A zero quaternion has no direction: the setter leaves the rotation alone.
        fromSetter.setRotation({x: 0, y: 0, z: 0, w: 0}, false);
        expect(fromSetter.rotation().y).toBeCloseTo(Math.SQRT1_2, 5);
        world.free();
    });

    test("bodies created in a restored world read correct transforms before its first step", () => {
        const world = new RAPIER.World(GRAVITY);
        world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(1, 2, 3));
        world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(4, 5, 6));
        world.step();

        const restored = RAPIER.World.restoreSnapshot(world.takeSnapshot())!;
        const spawned = restored.createRigidBody(
            RAPIER.RigidBodyDesc.fixed().setTranslation(7, 8, 9),
        );
        const spawnedCollider = restored.createCollider(
            RAPIER.ColliderDesc.ball(0.5).setTranslation(0.5, 0, 0),
            spawned,
        );
        ...
        expect(spawnedCollider.translation()).toEqual({x: 7.5, y: 8, z: 9});
        restored.step();
        expect(positions()).toEqual(expected);
        expect(spawnedCollider.translation()).toEqual({x: 7.5, y: 8, z: 9});
    ...
    test("bodies spawned between steps keep reading through the buffer", () => {
        const world = new RAPIER.World(GRAVITY);
        const first = world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(1, 1, 1));
        world.step();
        ...
        for (let i = 0; i < 200; i++) {
            spawned.push(
                world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(i, 0, 0)),
            );
            world.createCollider(RAPIER.ColliderDesc.ball(0.5).setTranslation(0, 1, 0), spawned[i]);
        }
        expect(first.translation()).toEqual({x: 1, y: 1, z: 1});
        for (let i = 0; i < 200; i++) {
            expect(spawned[i].translation()).toEqual({x: i, y: 0, z: 0});
            expect(spawned[i].collider(0)!.translation()).toEqual({x: i, y: 1, z: 0});
        }
        world.free();
    });
