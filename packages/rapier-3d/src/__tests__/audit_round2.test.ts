...
describe("setTransform", () => {
    ...
    test("wakes a sleeping body when only the rotation changes", () => {
        ...
        const body = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 5, 0).setSleeping(true),
        );
        world.createCollider(RAPIER.ColliderDesc.ball(0.5), body);
        expect(body.isSleeping()).toBe(true);

        body.setTransform(body.translation(), QUARTER_TURN_Y, true);
    ...
    test("wakes a sleeping body when only the translation changes", () => {
        ...
        body.setTransform({x: 1, y: 5, z: 0}, IDENTITY, true);
        expect(body.isSleeping()).toBe(false);
        expect(body.translation().x).toBeCloseTo(1);
        world.free();
    });

    test("wakes a sleeping body even when the quaternion is rejected", () => {
        const world = new RAPIER.World(GRAVITY);
        const body = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 5, 0).setSleeping(true),
        );
        ...
        body.setTransform(body.translation(), {x: 0, y: 0, z: 0, w: 0}, true);
        expect(body.isSleeping()).toBe(false);
        expect(body.rotation().w).toBeCloseTo(1);
        world.free();
    });
});
...
describe("invalid input is rejected instead of trapping the module", () => {
    ...
    test("ragged voxel data throws", () => {
        ...
        expect(() =>
            world.createCollider(RAPIER.ColliderDesc.voxels(new Int32Array([0, 0, 0, 1, 1]), size)),
        ).toThrow(/grid coordinates/);
        expect(() =>
            world.createCollider(RAPIER.ColliderDesc.voxels(new Float32Array([0, 0, 0, 1]), size)),
        ).toThrow(/points/);
        // Well-formed data still works.
        const ok = world.createCollider(
            RAPIER.ColliderDesc.voxels(new Int32Array([0, 0, 0, 1, 0, 0]), size),
        );
...
describe("quaternion normalization", () => {
    test("a shape query rotation off unit length is normalized", () => {
        const world = new RAPIER.World(GRAVITY);
        const ground = world.createRigidBody(RAPIER.RigidBodyDesc.fixed());
        world.createCollider(RAPIER.ColliderDesc.cuboid(10, 0.5, 10), ground);

        // Queries read the broad-phase, which a step populates.
        world.step();
        const shape = new RAPIER.Cuboid(0.5, 0.5, 0.5);
        const pos = {x: 0, y: 5, z: 0};
        const vel = {x: 0, y: -1, z: 0};
        const unit = world.castShape(pos, IDENTITY, vel, shape, 0, 100, true);
        // glam would apply this as a rotation scaled by |q|² = 0.25.
        const scaled = world.castShape(pos, {x: 0, y: 0, z: 0, w: 0.5}, vel, shape, 0, 100, true);
        expect(unit).not.toBeNull();
        expect(scaled).not.toBeNull();
        expect(unit!.time_of_impact).toBeCloseTo(4, 3);
        expect(scaled!.time_of_impact).toBeCloseTo(unit!.time_of_impact, 5);
        world.free();
    });
});
...
describe("character controller impulses", () => {
    test("the body rapier pushes reports its new velocity right away, even when it is not the hit one", () => {
        ...
        const characterCollider = world.createCollider(
            RAPIER.ColliderDesc.cuboid(1, 0.5, 0.5),
            character,
        );
        ...
        const boxes = [
            ...
            world.createCollider(RAPIER.ColliderDesc.cuboid(0.5, 0.25, 0.5).setDensity(1), body);
            return body;
        });
...
describe("transform sync with few bodies and many colliders", () => {
    ...
    test("standalone and JS-moved colliders stay correct", () => {
        ...
        for (let i = 0; i < 200; ++i) {
            standalone.push(
                world.createCollider(RAPIER.ColliderDesc.ball(0.1).setTranslation(i, 0, 0)),
            );
        }
        const bodies: RAPIER.RigidBody[] = [];
        for (let i = 0; i < 4; ++i) {
            const body = world.createRigidBody(
                RAPIER.RigidBodyDesc.dynamic().setTranslation(i, 10, 0),
            );
        ...
        standalone[7].setTranslation({x: 500, y: 1, z: 2});
        bodies[2].setTranslation({x: 42, y: 42, z: 42}, true);
        ...
        world.step();
        expect(standalone[7].translation()).toEqual({x: 500, y: 1, z: 2});
        ...
        for (let i = 0; i < 50; ++i) {
            colliders.push(
                world.createCollider(
                    RAPIER.ColliderDesc.cuboid(0.5, 0.5, 0.5).setTranslation(0, i, 0),
                ),
            );
        }
        world.step();
        colliders[3].setTranslation({x: 1, y: 2, z: 3});
        world.step();
        for (let i = 0; i < 50; ++i) {
            const expected = i === 3 ? {x: 1, y: 2, z: 3} : {x: 0, y: i, z: 0};
