...
function groundAndBox(world: RAPIER.World, hooks: RAPIER.ActiveHooks) {
    ...
    world.createCollider(RAPIER.ColliderDesc.cuboid(10, 0.5, 10).setActiveHooks(hooks), ground);
    const box = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 1, 0));
...
describe("exceptions thrown from callbacks passed to the phases directly", () => {
    ...
    test("NarrowPhase.contactPairsWith stops and propagates", () => {
        ...
        expect(calls).toBe(1);
        // The walk is usable afterwards and honours a `false` from the callback.
        let seen = 0;
        world.narrowPhase.contactPairsWith(colliders[0].handle, () => {
            seen += 1;
        });
...
describe("shape-specific collider getters", () => {
    test("return null for a collider of another shape", () => {
        ...
        expect(ball.heightfieldNRows()).toBeNull();
        expect(ball.heightfieldNCols()).toBeNull();
...
describe("vehicle controller", () => {
    test("wheel vectors round-trip through the scalar setters", () => {
        const world = new RAPIER.World(GRAVITY);
        ...
        vehicle.setWheelChassisConnectionPointCs(0, {x: -1, y: -0.5, z: 2});
        vehicle.setWheelDirectionCs(0, {x: 0, y: -0.5, z: 0});
        vehicle.setWheelAxleCs(0, {x: 0, y: 0, z: 1});
        expect(vehicle.wheelChassisConnectionPointCs(0)).toEqual({x: -1, y: -0.5, z: 2});
        expect(vehicle.wheelDirectionCs(0)).toEqual({x: 0, y: -0.5, z: 0});
        expect(vehicle.wheelAxleCs(0)).toEqual({x: 0, y: 0, z: 1});

        // Out-of-range wheels are still ignored rather than trapping.
        vehicle.setWheelAxleCs(7, {x: 0, y: 0, z: 1});
        expect(vehicle.wheelAxleCs(7)).toBeNull();
        vehicle.free();
        world.free();
    });
});
...
describe("event queue without auto-drain", () => {
    function fallingBall() {
        ...
        world.createCollider(
            RAPIER.ColliderDesc.cuboid(10, 0.5, 10).setActiveEvents(
                RAPIER.ActiveEvents.COLLISION_EVENTS,
            ),
            ground,
        );
        const body = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 1.2, 0),
        );
    ...
    test("a handler that grows WASM memory inside the drain sees the whole batch", () => {
        ...
        queue.drainCollisionEvents(() => {
            ...
            // is still going.
            for (let i = 0; i < 200; ++i) {
                world.createCollider(RAPIER.ColliderDesc.ball(0.1).setTranslation(50 + i, 50, 0));
            }
        });
        expect(seen).toBe(1);
        world.free();
