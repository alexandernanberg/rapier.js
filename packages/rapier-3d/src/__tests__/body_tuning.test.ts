...
describe("body tuning", () => {
    test("locked translations pin the body in place", () => {
        ...
        const body = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 5, 0).lockTranslations(),
        );
    ...
    test("locked rotations keep the body's orientation at identity", () => {
        const world = new RAPIER.World(GRAVITY);
        const body = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 5, 0).lockRotations(),
        );
        world.createCollider(RAPIER.ColliderDesc.cuboid(0.5, 0.5, 0.5), body);
        body.applyTorqueImpulse({x: 0, y: 0, z: 50}, true);
    ...
    test("gravityScale scales how fast a body falls", () => {
        ...
        const slow = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 5, 0).setGravityScale(0.5),
        );
        const fast = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(2, 5, 0).setGravityScale(2),
        );
    ...
    test("linear damping slows a body down", () => {
        ...
        const damped = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setLinvel(10, 0, 0).setLinearDamping(5),
        );
        const free = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 5, 0).setLinvel(10, 0, 0),
        );
...
describe("continuous collision detection", () => {
    test("a fast body does not tunnel through a thin wall when CCD is on", () => {
        ...
        world.createCollider(RAPIER.ColliderDesc.cuboid(0.05, 5, 5), wall);
...
describe("debug render", () => {
    test("debugRender returns one colour per vertex", () => {
        ...
        // 3 floats per vertex in 3D, 4 colour components per vertex.
        expect(buffers.colors.length / 4).toBe(buffers.vertices.length / 3);
...
describe("handle lifetime", () => {
    test("a stale handle resolves to the body that recycled its slot", () => {
        const world = new RAPIER.World(GRAVITY);
        const first = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(1, 1, 1));
        ...
        const second = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(9, 9, 9),
        );
