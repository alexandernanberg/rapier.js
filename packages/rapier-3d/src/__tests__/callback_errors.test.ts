...
function stackedBoxes(world: RAPIER.World, count: number): RAPIER.Collider[] {
    ...
    for (let i = 0; i < count; ++i) {
        const body = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(0, i, 0));
        colliders.push(world.createCollider(RAPIER.ColliderDesc.cuboid(0.5, 0.5, 0.5), body));
    }
    return colliders;
}
...
describe("exceptions thrown from query callbacks", () => {
    ...
    test("a throwing callback propagates out of the shape and point queries", () => {
        ...
        expect(() =>
            world.intersectionsWithShape({x: 0, y: 1, z: 0}, {x: 0, y: 0, z: 0, w: 1}, shape, fail),
        ).toThrow("boom");
        expect(() => world.intersectionsWithPoint({x: 0, y: 1, z: 0}, fail)).toThrow("boom");
        expect(() =>
            world.collidersWithAabbIntersectingAabb({x: 0, y: 1, z: 0}, {x: 1, y: 1, z: 1}, fail),
        ).toThrow("boom");
        expect(() =>
            world.projectPoint(
                {x: 0, y: 1, z: 0},
        ...
        expect(
            world.castRay(new RAPIER.Ray({x: 0, y: 10, z: 0}, {x: 0, y: -1, z: 0}), 100, true),
        ).not.toBeNull();
        world.step();
        world.free();
    });
