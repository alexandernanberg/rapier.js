...
function groundedCharacter() {
    ...
    const body = world.createRigidBody(
        RAPIER.RigidBodyDesc.kinematicPositionBased().setTranslation(0, 1.5, 0),
    );
    const collider = world.createCollider(RAPIER.ColliderDesc.cuboid(0.3, 0.5, 0.3), body);
    world.step();
...
describe("kinematic character controller", () => {
    test("horizontal movement is preserved and the character is grounded", () => {
        ...
        controller.computeColliderMovement(collider, {x: 1, y: -0.5, z: 0});
    ...
    test("a wall blocks movement into it", () => {
        ...
        const wallBody = world.createRigidBody(
            RAPIER.RigidBodyDesc.fixed().setTranslation(1.5, 1, 0),
        );
        world.createCollider(RAPIER.ColliderDesc.cuboid(0.1, 2, 5), wallBody);
        world.step();

        const controller = world.createCharacterController(0.01);
        controller.computeColliderMovement(collider, {x: 5, y: 0, z: 0});
    ...
    test("up() returns a plain vector", () => {
        ...
        expect(controller.up()).toEqual({x: 0, y: 1, z: 0});
    ...
    test("impulses applied to pushed bodies are visible before the next step", () => {
        ...
        const box = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(0.9, 1.5, 0),
        );
        world.createCollider(RAPIER.ColliderDesc.cuboid(0.3, 0.5, 0.3), box);
        world.step();
