...
describe("event drain buffers", () => {
    function collidingScene(bodies: number) {
        ...
        const groundCollider = world.createCollider(
            RAPIER.ColliderDesc.cuboid(50, 0.5, 50).setActiveEvents(
        ...
        for (let i = 0; i < bodies; i++) {
            const body = world.createRigidBody(
                RAPIER.RigidBodyDesc.dynamic().setTranslation(i * 2, 1.5, 0),
            );
            colliders.push(
    ...
    test("a recycled collider slot keeps its generation through the buffer", () => {
        ...
        const body = world.createRigidBody(
            RAPIER.RigidBodyDesc.dynamic().setTranslation(0, 1.5, 0),
        );
