...
describe("integration parameters", () => {
    ...
    test("the friction model can be switched", () => {
        const world = new RAPIER.World(GRAVITY);
        const params = world.integrationParameters;

        expect(params.frictionModel).toBe(RAPIER.FrictionModel.Simplified);

        params.frictionModel = RAPIER.FrictionModel.Coulomb;
        expect(params.frictionModel).toBe(RAPIER.FrictionModel.Coulomb);
    ...
    test("a box still comes to rest with non-default tuning", () => {
        const world = new RAPIER.World(GRAVITY);
        world.integrationParameters.frictionModel = RAPIER.FrictionModel.Coulomb;
