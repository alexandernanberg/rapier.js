...
describe("multibody joint properties", () => {
    ...
    function armWorld(gravity = GRAVITY) {
        const world = new RAPIER.World(gravity);
        const anchor = world.createRigidBody(RAPIER.RigidBodyDesc.fixed().setTranslation(0, 5, 0));
        const arm = world.createRigidBody(RAPIER.RigidBodyDesc.dynamic().setTranslation(1, 5, 0));
    ...
    test("a multibody joint reports its bodies, type and anchors", () => {
        ...
        expect(joint.anchor1(_v())).toEqual({x: 0, y: 0, z: 0});
        expect(joint.anchor2(_v())).toEqual({x: -1, y: 0, z: 0});
        expect(joint.frameX1(_q())).not.toBeNull();

        // The zero-allocation form writes into the object it is given.
        const target = {x: 0, y: 0, z: 0};
        expect(joint.anchor2(target)).toBe(target);
        expect(target).toEqual({x: -1, y: 0, z: 0});
    ...
    test("a velocity motor drives the arm around its axis", () => {
        const {world, arm, joint} = armWorld({x: 0, y: 0, z: 0});
        ...
        // Turning about +z with the arm out along +x lifts it.
