...
describe("scene queries", () => {
    ...
    test("castRayAndGetNormal also reports the normal at the hit point", () => {
        ...
        expect(hit!.normal.z).toBeCloseTo(0, 4);
    ...
    test("intersectionsWithRay stops when the callback returns false", () => {
        const world = new RAPIER.World({x: 0, y: 0, z: 0});
        for (const y of [0, 5, 10]) {
            const body = world.createRigidBody(
                RAPIER.RigidBodyDesc.fixed().setTranslation(0, y, 0),
        ...
        world.intersectionsWithRay(
            new RAPIER.Ray({x: 0, y: 20, z: 0}, {x: 0, y: -1, z: 0}),
            100,
            true,
            () => {
                count += 1;
    ...
    test("projectPointAndGetFeature reports the projected-on feature", () => {
        ...
        expect(proj!.featureType).toBe(RAPIER.FeatureType.Face);
    ...
    test("intersectionsWithPoint reports the colliders containing the point", () => {
        ...
        const hits: number[] = [];
        world.intersectionsWithPoint({x: 0, y: 0, z: 0}, (found) => {
        ...
        hits.length = 0;
        world.intersectionsWithPoint({x: 20, y: 0, z: 0}, (found) => {
            hits.push(found.handle);
            return true;
        });
        expect(hits).toEqual([]);
